import { AdoptionShellValidationError, CONTRACT_ID, CONTRACT_VERSION, ENVELOPE_DIGEST_ALGORITHM, PAYLOAD_DIGEST_ALGORITHM, SCHEMA_DIGESTS, SCHEMA_IDS, } from "./contract.js";
...
function selectedEntryDiagnostics(input, selectedBundleIds, selectedDeclaredPaths) {
    ...
    for (const entry of input.release.entries) {
        ...
        if (selectedBundleIds.has(entry.bundleId) &&
...
export function materializeAdoptionShellV2(inputValue) {
    ...
    const closure = resolveCapabilityClosure(input.capabilities, input.requestedBundles, input.release.entries);
    const bundleIds = new Set(closure.bundles.map((bundle) => bundle.id));
    const declaredPaths = new Map(closure.bundles.map((bundle) => [
        bundle.id,
    ...
    const selectedBundles = Object.freeze(closure.bundles.map((bundle) => Object.freeze({
        id: bundle.id,
    ...
    const migrationRefs = Object.freeze([]);
    const manifestBody = {
