    ...
    for (const entry of entries) {
        ...
        for (const match of text.matchAll(/\[([^\]]+)\]\(([^)\s]+)\)/g)) {
            ...
            if (resolved === null || !byPath.has(resolved)) {
                diagnostics.add("E_DOC_LINK_MISSING", `/entries/${entry.path}`, `relative documentation link does not resolve: ${link}`);
                continue;
            }
            if (/^ADR-\d{4}: /.test(label)) {
                ...
                if (target?.encoding === "utf-8") {
                    ...
                    if (heading !== `# ${label}`) {
                        diagnostics.add("E_DOC_ADR_TITLE", `/entries/${entry.path}`, `linked ADR heading does not match ${label}`);
                    }
                }
            }
        }
