...
export const REPOSITORY_SHAPE_PROFILE_SCHEMA_DIGEST = "0c349cb61509427ead06280d57c3bbfea7d6010e42e27bf653e4ddd2a37772ed";
export const TURBO_TASK_GRAPH_SCHEMA_DIGEST = "4870a7cc7642fd2856ca7eac45424b59140ff6f2248f5af74f5ada1490e9778c";
export function validateRepositoryShapeProfileWire(value, diagnostics) {
    if (!isRecord(value)) {
        diagnostics.add("E_TYPE", "", "expected object");
        return false;
    }
    const allowed = new Set(["$schema", "$schema", "schemaId", "schemaVersion", "contractId", "profileId", "monorepo", "rootFamilies", "declaredScripts"]);
    for (const key of Object.keys(value)) {
        if (!allowed.has(key)) {
            diagnostics.add("E_UNKNOWN_PROPERTY", `/${escapePointer(key)}`, "unknown property");
        }
    }
    for (const key of ["schemaId", "schemaVersion", "contractId", "profileId", "monorepo", "rootFamilies", "declaredScripts"]) {
        if (!Object.hasOwn(value, key)) {
            diagnostics.add("E_REQUIRED", `/${escapePointer(key)}`, "required property is missing");
        }
    }
    ...
    if (Object.hasOwn(value, "schemaId") && value["schemaId"] !== "https://schemas.repo-template.dev/repository-shape/v1/repository-shape.schema.json") {
        diagnostics.add("E_CONST", "/schemaId", "must equal \"https://schemas.repo-template.dev/repository-shape/v1/repository-shape.schema.json\"");
    }
    if (Object.hasOwn(value, "schemaVersion") &&
        value["schemaVersion"] !== "1.0.0") {
        diagnostics.add("E_CONST", "/schemaVersion", "must equal \"1.0.0\"");
    }
    if (Object.hasOwn(value, "contractId") &&
        value["contractId"] !== "repo-template/repository-shape/v1") {
        diagnostics.add("E_CONST", "/contractId", "must equal \"repo-template/repository-shape/v1\"");
    }
    ...
    if (Object.hasOwn(value, "profileId")) {
        if (typeof profileId !== "string") {
            ...
            if (profileId.length < 1) {
                diagnostics.add("E_LENGTH", "/profileId", "must contain at least 1 characters");
            }
            if (profileId.length > 80) {
                diagnostics.add("E_LENGTH", "/profileId", "must contain at most 80 characters");
            }
            if (!/^[a-z0-9]+(?:-[a-z0-9]+)*$/.test(profileId)) {
                diagnostics.add("E_FORMAT", "/profileId", "string does not match the required format");
            }
        }
    }
    ...
    if (Object.hasOwn(value, "rootFamilies")) {
        if (!Array.isArray(rootsVal)) {
            ...
            const allowedRoots = new Set(["apps", "services", "native", "tools", "packages", "database", "infrastructure"]);
            ...
            for (const [idx, root] of rootsVal.entries()) {
                ...
                if (typeof root !== "string" || !allowedRoots.has(root)) {
                    diagnostics.add("E_INVALID_ROOT", pointer, `invalid root family: ${String(root)}`);
                }
                else if (seenRoots.has(root)) {
                    diagnostics.add("E_DUPLICATE_ROOT", pointer, `duplicate root family: ${root}`);
                }
                else {
                    seenRoots.add(root);
                }
    ...
    if (Object.hasOwn(value, "declaredScripts")) {
        if (!Array.isArray(scriptsVal)) {
            diagnostics.add("E_TYPE", "/declaredScripts", "declaredScripts must be an array");
        }
        else {
            ...
            for (const [idx, script] of scriptsVal.entries()) {
                ...
                if (typeof script !== "string" || !scriptPattern.test(script)) {
                    diagnostics.add("E_INVALID_SCRIPT", pointer, `invalid script: ${String(script)}`);
                }
                else if (seenScripts.has(script)) {
                    diagnostics.add("E_DUPLICATE_SCRIPT", pointer, `duplicate declared script: ${script}`);
                }
                else {
...
export function validateTurboTaskGraphWire(value, diagnostics) {
    if (!isRecord(value)) {
        diagnostics.add("E_TYPE", "", "expected object");
        return false;
    }
    const allowed = new Set(["$schema", "tasks"]);
    for (const key of Object.keys(value)) {
        if (!allowed.has(key)) {
            diagnostics.add("E_UNKNOWN_PROPERTY", `/${escapePointer(key)}`, "unknown property");
        }
    ...
    if (Object.hasOwn(value, "tasks")) {
        ...
        for (const [taskName, taskDefinition] of Object.entries(tasksVal)) {
            ...
            const allowedTaskKeys = new Set(["dependsOn", "outputs", "cache", "inputs", "persistent"]);
            for (const key of Object.keys(taskDefinition)) {
                if (!allowedTaskKeys.has(key)) {
                    diagnostics.add("E_UNKNOWN_PROPERTY", `${taskPointer}/${escapePointer(key)}`, "unknown property");
                }
            }
                ...
                if (!Array.isArray(inputs)) {
                    ...
                    for (const [idx, input] of inputs.entries()) {
                        if (typeof input !== "string") {
                            diagnostics.add("E_TYPE", `${taskPointer}/inputs/${String(idx)}`, "expected string");
                        }
                    }
                }
            }
