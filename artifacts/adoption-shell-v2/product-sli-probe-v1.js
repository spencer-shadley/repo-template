...
export const PRODUCT_SLI_PROBE_CONTRACT_ID = "repo-template/product-sli-probe-v1";
export const PRODUCT_SLI_PROBE_SCHEMA_VERSION = "product-sli-probe/v1";
export const PRODUCT_SLI_PROBE_SCHEMA_ID = "https://schemas.repo-template.dev/product-sli-probe/v1/product-sli-probe.schema.json";
export const PRODUCT_SLI_OBSERVATION_SCHEMA_VERSION = "product-sli-observation/v1";
export const PRODUCT_SLI_OBSERVATION_SCHEMA_ID = "https://schemas.repo-template.dev/product-sli-probe/v1/product-sli-observation.schema.json";
export const PRODUCT_SLI_MIGRATION_RECEIPT_SCHEMA_VERSION = "product-sli-migration-receipt/v1";
 ...
 */
export function parsePrioritiesLocalSliTable(prioritiesMarkdown) {
    ...
    for (let index = headerRowIndex + 2; index < lines.length; index += 1) {
        ...
        if (/^##\s+/u.test(line))
            break;
        const cells = parseMarkdownTableCells(line);
        if (!cells || cells.length <= Math.max(idCol, principleCol, sliCol, sloCol, statusCol))
            continue;
...
function validateEntrypoint(entrypoint, pointer, diagnostics) {
    const allowed = ["path", "argv", "cwd", "timeoutSeconds"];
    if (!diagnostics.object(entrypoint, pointer, allowed, ["path", "argv"]))
        return;
    ...
    if (diagnostics.array(argvVal, `${pointer}/argv`, 0, 128)) {
        for (const [argIndex, arg] of argvVal.entries()) {
            if (typeof arg !== "string") {
                ...
                diagnostics.add("E_SHELL_INJECTION", `${pointer}/argv/${String(argIndex)}`, "argument contains forbidden shell metacharacters");
            }
        }
    }
...
function validateEffects(effects, pointer, diagnostics) {
    const allowed = ["effectClass", "spend", "liveMutation"];
    if (!diagnostics.object(effects, pointer, allowed, ["effectClass"]))
        return;
    const effectClass = effects["effectClass"];
    if (typeof effectClass === "string") {
        if (FORBIDDEN_EFFECT_CLASSES.has(effectClass)) {
            diagnostics.add("E_FORBIDDEN_EFFECT", `${pointer}/effectClass`, `forbidden effect class '${effectClass}': destructive, live-write, and spend are structurally refused`);
        }
...
function validateDisposition(disposition, pointer, diagnostics) {
    const fields = ["kind", "owner", "reviewDate", "rationale"];
    if (!diagnostics.object(disposition, pointer, fields, fields))
        return;
...
function validateSingleProbe(probe, pointer, diagnostics, seenIds) {
    const allowed = [
        "sliId",
        "bindsPrinciple",
        "kind",
    ...
    if (!diagnostics.object(probe, pointer, allowed, required))
        return;
    ...
    if (probe["receipt"] !== undefined) {
        ...
        if (diagnostics.object(rcpt, `${pointer}/receipt`, ["schemaVersion", "evidenceKind"], ["schemaVersion"])) {
            if (rcpt["schemaVersion"] !== PRODUCT_SLI_OBSERVATION_SCHEMA_VERSION) {
                diagnostics.add("E_CONST", `${pointer}/receipt/schemaVersion`, `expected '${PRODUCT_SLI_OBSERVATION_SCHEMA_VERSION}'`);
            }
        }
    }
}
 ...
 */
export function validateProductSliProbeContractV1(contractRaw, prioritiesMarkdown) {
    ...
    if (prioritiesMarkdown !== undefined && diagnostics.rows.length === 0) {
        ...
        for (const row of rows) {
            if (row.id.startsWith("_(") && row.id.endsWith(")_"))
        ...
        for (const row of rows) {
            ...
            if (row.status.toLowerCase() === "active") {
                ...
                if (!probe) {
                    diagnostics.add("E_UNBOUND_ACTIVE_ROW", `/probes/${row.id}`, `active local row '${row.id}' in PRIORITIES.md has no probe binding or explicit report-only disposition`);
                }
                else {
                    const principle = probe["bindsPrinciple"];
                    if (typeof principle === "string" && principle !== row.bindsPrinciple) {
                        diagnostics.add("E_PRINCIPLE_MISMATCH", `/probes/${row.id}/bindsPrinciple`, `probe '${row.id}' binds principle '${principle}' which does not match PRIORITIES.md principle '${row.bindsPrinciple}'`);
                    }
                }
            }
        }
        ...
        for (const [id, probe] of probesById.entries()) {
            ...
            if (!row) {
                diagnostics.add("E_UNKNOWN_SLI_ROW", `/probes/${id}`, `probe specifies sliId '${id}' which is not present in PRIORITIES.md`);
            }
            else {
                const principle = probe["bindsPrinciple"];
                if (typeof principle === "string" && principle !== row.bindsPrinciple) {
                    diagnostics.add("E_PRINCIPLE_MISMATCH", `/probes/${id}/bindsPrinciple`, `probe '${id}' binds principle '${principle}' which does not match PRIORITIES.md principle '${row.bindsPrinciple}'`);
                }
            }
        }
    }
    ...
    return { ok: true, value: contractRaw };
}
/**
 * Validate typed observation payload
 */
export function validateProductSliObservationV1(value) {
    const diagnostics = new Diagnostics();
    const allowed = [
        "schemaId",
        "schemaVersion",
...
export function createProductSliMigrationReceipt(params) {
    ...
    const bindings = params.prioritiesRows
        ...
        return {
            ...
            probeKind: probe?.kind ?? "report-only",
            sliId: row.id,
            status: row.status,
            target: row.slo,
        };
