...
function validateOutputEntry(value, pointer, diagnostics) {
    ...
    if (generic ? value["bundleId"] !== null : typeof value["bundleId"] !== "string") {
        diagnostics.add("E_BUNDLE_OWNERSHIP", `${pointer}/bundleId`, "bundleId does not agree with the entry role");
    }
...
export function validateMaterializerOutputManifestV2(value) {
    ...
    schemaIdentity(value, SCHEMA_IDS.materializerOutputManifest, SCHEMA_DIGESTS.materializerOutputManifest, diagnostics);
    ...
    if (diagnostics.array(value["selectedBundles"], "/selectedBundles", 0, 256)) {
        ...
        value["selectedBundles"].forEach((reference, index) => {
            ...
            if (diagnostics.object(reference, pointer, ["id", "version", "digest"], ["id", "version", "digest"])) {
                ...
                keys.push(`${String(reference["id"])}\u0000${String(reference["version"])}\u0000${String(reference["digest"])}`);
            }
        });
...
export function validateArtifactManifestV2(value) {
    ...
    schemaIdentity(value, SCHEMA_IDS.artifactManifest, SCHEMA_DIGESTS.artifactManifest, diagnostics);
    diagnostics.string(value["contractId"], "/contractId", { constant: CONTRACT_ID });
    ...
    if (diagnostics.object(value["toolchain"], "/toolchain", ["typescript", "nodeCompatibility", "packageManager"], ["typescript", "nodeCompatibility", "packageManager"])) {
        diagnostics.string(value["toolchain"]["typescript"], "/toolchain/typescript", {
            constant: "7.0.2",
        });
        diagnostics.string(value["toolchain"]["nodeCompatibility"], "/toolchain/nodeCompatibility", { constant: ">=24.16.0 <25" });
