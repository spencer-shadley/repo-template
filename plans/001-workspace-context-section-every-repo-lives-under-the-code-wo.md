...
3. **CHANGELOG.md** — add under a new `## [the next MINOR above the current TEMPLATE_VERSION at implementation time] - <today>` heading: "Added: workspace-context
   section (nesting contract + inherited pieces)".
4. **TEMPLATE_VERSION** — set to `the next MINOR above the current TEMPLATE_VERSION at implementation time` (MINOR: additive doc structure).
...
- [ ] TEMPLATE_VERSION == the next MINOR above the current TEMPLATE_VERSION at implementation time and CHANGELOG has the the next MINOR above the current TEMPLATE_VERSION at implementation time heading.
- [ ] Verify gate green.
