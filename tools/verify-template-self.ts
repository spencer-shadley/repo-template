...
function gitBlobId(content: Uint8Array): string {
  const header = Buffer.from(`blob ${content.byteLength.toString()}\0`, "utf8");
  ...
  fs.readFileSync(path.join(root, ...relativePath.split("/")), "utf8")
    ...
    .forEach((line, index) => {
      if (isConflictMarker(line)) {
        conflicts.push(`${relativePath}:${(index + 1).toString()}:${line}`);
      }
    });
}
...
if (process.argv.includes("--portable-charter")) {
  if (portableCharterErrors.length > 0) {
    ...
    console.log(
      `portable charter: ${portableCharterHeadings.length.toString()} headings, placeholders, priorities pointer, and removal checks passed`,
    );
  }
  ...
  if (directL0DefaultErrors.length > 0) {
    ...
    console.log(
      `direct-L0 defaults: ${requiredDirectL0Defaults.length.toString()} clauses and removal checks passed`,
    );
  }
} else if (
  conflicts.length > 0 ||
