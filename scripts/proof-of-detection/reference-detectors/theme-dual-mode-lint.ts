...
interface DarkSurfaceFinding {
  readonly index: number;
  readonly value: string;
  readonly luminance: number;
  readonly kind: 'hex' | 'rgb';
}
...
function scanForDarkSurfaces(
  cssText: string,
  { mode = 'luminance' }: { readonly mode?: string } = {},
): DarkSurfaceFinding[] {
  const findings: DarkSurfaceFinding[] = [];
...
function runCli(filePath: string, mode: string): number {
  ...
  console.error(`theme-dual-mode-lint(${mode}): darkFingerprintCss: ${String(findings.length)}`);
...
if (invokedAsMain) {
  ...
  if (!filePath) {
    console.error('usage: node theme-dual-mode-lint.ts <css-file> [--mode=luminance|hex-only]');
    process.exit(2);
  }
