...
export function validateTemplateReleaseClosureV1(
  ...
  const compare = (
    ...
    if (actual !== expected) diagnostics.add(code, pointer, message);
  };
  compare(
    receipt.payloadSet.manifestDigest,
    payload.releaseDigest,
    "/receipt/payloadSet/manifestDigest",
    "E_RELEASE_MANIFEST_MISMATCH",
    "receipt does not bind the supplied release payload-set manifest",
  );
  compare(
    receipt.payloadSet.payloadDigest,
    payload.payloadDigest,
    "/receipt/payloadSet/payloadDigest",
    "E_RELEASE_PAYLOAD_MISMATCH",
    "receipt does not bind the supplied release payload digest",
  );
  compare(
    receipt.payloadSet.entryCount,
    payload.entryCount,
    "/receipt/payloadSet/entryCount",
    "E_RELEASE_ENTRY_COUNT_MISMATCH",
    "receipt does not bind the supplied release entry count",
  );
  compare(
    receipt.materializer.artifactManifestDigest,
    artifact.manifestDigest,
    "/receipt/materializer/artifactManifestDigest",
    "E_ARTIFACT_MANIFEST_MISMATCH",
    "receipt does not bind the supplied artifact manifest",
  );
  compare(
    ...
    "/receipt/materializer/artifactDigest",
    "E_ARTIFACT_MISMATCH",
    "receipt does not bind the supplied compiled artifact",
  );
  compare(
    receipt.receiptKind,
    artifact.releaseReceiptKind,
    "/receipt/receiptKind",
    "E_RELEASE_KIND_MISMATCH",
    "artifact manifest does not declare the receipt kind",
  );
