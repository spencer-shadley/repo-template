...
export function validateTemplateReleaseReceiptV1(
  ...
  if (
    ...
    if (
      diagnostics.string(value["producer"]["semver"], "/producer/semver", {
        min: 5,
        max: 80,
        pattern: SEMVER_PATTERN,
      })
    ...
    if (
      diagnostics.string(value["producer"]["tag"], "/producer/tag", {
        min: 6,
        max: 81,
      })
    ) {
  ...
  if (
    diagnostics.object(
      value["receiptTransport"],
      "/receiptTransport",
      ["kind", "tagName", "targetObjectType", "bodyEncoding", "bodyCanonicalization"],
      ["kind", "tagName", "targetObjectType", "bodyEncoding", "bodyCanonicalization"],
    )
  ) {
    ...
    if (
      diagnostics.string(
        value["receiptTransport"]["tagName"],
    ...
    diagnostics.string(
      value["receiptTransport"]["targetObjectType"],
      "/receiptTransport/targetObjectType",
      { constant: "commit" },
    );
    diagnostics.string(
      value["receiptTransport"]["bodyEncoding"],
      "/receiptTransport/bodyEncoding",
      { constant: "utf-8" },
    );
    diagnostics.string(
      value["receiptTransport"]["bodyCanonicalization"],
      "/receiptTransport/bodyCanonicalization",
      { constant: "rfc8785" },
    );
  }
  ...
  if (
    ...
    if (
      diagnostics.sha(value["payloadSet"]["schemaDigest"], "/payloadSet/schemaDigest") &&
    ...
    diagnostics.string(
      value["payloadSet"]["payloadDigestAlgorithm"],
    ...
    if (
      !Number.isInteger(value["payloadSet"]["entryCount"]) ||
  ...
  if (
    ...
    diagnostics.string(
      materializer["artifactManifestPath"],
      "/materializer/artifactManifestPath",
      { constant: ARTIFACT_MANIFEST_PATH },
    );
    diagnostics.string(
      materializer["artifactManifestSchemaId"],
      "/materializer/artifactManifestSchemaId",
      { constant: SCHEMA_IDS.artifactManifest },
    );
    diagnostics.string(
      materializer["artifactManifestSchemaVersion"],
      "/materializer/artifactManifestSchemaVersion",
      { constant: CONTRACT_VERSION },
    );
    if (
    ...
    diagnostics.sha(
      materializer["artifactManifestDigest"],
      "/materializer/artifactManifestDigest",
    );
    ...
    diagnostics.string(
      materializer["runtimeCompatibility"],
      "/materializer/runtimeCompatibility",
      { constant: ">=24.16.0 <25" },
    );
    diagnostics.string(
