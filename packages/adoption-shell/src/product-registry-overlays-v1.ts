...
export const TECHNOLOGY_REGISTRY_OVERLAY_CONTRACT_ID =
  "repo-template/technology-registry-overlay-v1" as const;
export const COMPONENT_REGISTRY_OVERLAY_CONTRACT_ID =
  "repo-template/component-registry-overlay-v1" as const;

export const PRODUCT_OVERLAY_SCHEMA_ID =
  "https://schemas.repo-template.dev/overlays/v1/product-overlay.schema.json" as const;
export const TECHNOLOGY_REGISTRY_OVERLAY_SCHEMA_ID =
  "https://schemas.repo-template.dev/overlays/v1/technology-registry.overlay.schema.json" as const;
export const COMPONENT_REGISTRY_OVERLAY_SCHEMA_ID =
  "https://schemas.repo-template.dev/overlays/v1/component-registry.overlay.schema.json" as const;

export const OVERLAY_CAPABILITY_BUNDLE_ID =
  "repo-template/product-registry-overlays" as const;

export const PRODUCT_OVERLAY_FILE_PATH = "product-overlay.yaml" as const;
export const TECHNOLOGY_REGISTRY_OVERLAY_FILE_PATH =
  "technology-registry.overlay.yaml" as const;
export const COMPONENT_REGISTRY_OVERLAY_FILE_PATH =
...
export type ProductOverlayRole =
  | "primary"
  | "secondary"
  | "accessory"
  | "specialized"
...
const MUTABLE_BRANCH_PATTERN =
  /(?:^|\/|@|refs\/heads\/)(?:master|main|HEAD)(?:$|\/|\?|#)/i;
 ...
 */
export function isMutableBranchReference(value: string): boolean {
  if (MUTABLE_BRANCH_PATTERN.test(value)) return true;
  const trimmed = value.trim();
  if (trimmed === "master" || trimmed === "main" || trimmed === "HEAD") return true;
...
function finishResult<T>(value: unknown, diagnostics: Diagnostics): ValidationResult<T> {
...
export function validateBootstrapProvenance(
...
): value is BootstrapProvenance {
  if (
    !diagnostics.object(
      value,
      pointer,
      ["templateRelease", "bootstrappedFromGuideVersion", "lastAuditedAgainstGuideVersion"],
      ["templateRelease"],
    )
  ) {
    return false;
  ...
  if (
    !diagnostics.object(
      tr,
      `${pointer}/templateRelease`,
      ["repository", "semver", "releaseDigest", "releaseId", "receiptDigest"],
      ["repository", "semver", "releaseDigest"],
    )
  ) {
    return false;
  }
  ...
  if (typeof tr["semver"] === "string") {
    if (isMutableBranchReference(tr["semver"])) {
      diagnostics.add(
        ...
        `${pointer}/templateRelease/semver`,
        "mutable branch reference forbidden in release semver",
      );
    } else if (!SEMVER_PATTERN.test(tr["semver"])) {
      diagnostics.add(
        "E_FORMAT",
        `${pointer}/templateRelease/semver`,
        "string does not match the required SemVer format",
      );
    }
  } else {
  ...
  if (tr["releaseId"] !== undefined) {
    if (typeof tr["releaseId"] === "string") {
      if (isMutableBranchReference(tr["releaseId"])) {
        diagnostics.add(
          "E_PROVENANCE_INCOMPATIBLE",
          `${pointer}/templateRelease/releaseId`,
          "mutable branch reference forbidden in releaseId",
        );
      }
    } else {
    ...
    if (guideVal !== undefined) {
      if (typeof guideVal !== "string" || guideVal.trim().length === 0) {
        ...
        diagnostics.add(
          "E_PROVENANCE_INCOMPATIBLE",
          `${pointer}/${guideKey}`,
          `bootstrap provenance ${guideKey} cannot use mutable branch reference: "${guideVal}"`,
        );
      }
    }
  }
...
export function validateProductOverlay(
  value: unknown,
  pointer = "",
): ValidationResult<ProductOverlay> {
  const diagnostics = new Diagnostics();
  if (
    !diagnostics.object(
      value,
      pointer,
      ["schemaVersion", "contractId", "profile", "provenance", "platforms"],
      ["schemaVersion", "contractId", "profile", "provenance", "platforms"],
    )
  ) {
  ...
  if (schemaVersion !== "1.0.0" && schemaVersion !== "product-overlay/v1") {
    diagnostics.add(
      ...
      'schemaVersion must equal "product-overlay/v1" or "1.0.0"',
    );
  }
  ...
  if (platformKeys.length === 0) {
    diagnostics.add(
      "E_REQUIRED",
      `${pointer}/platforms`,
      "at least one platform record is required",
    );
  }
  ...
  for (const platformId of platformKeys) {
    ...
    if (typeof role !== "string" || !PRODUCT_OVERLAY_ROLES.has(role as ProductOverlayRole)) {
      diagnostics.add(
        "E_OVERLAY_ROLE",
        `${pPtr}/role`,
        `unknown or malformed platform role: "${String(role)}"`,
      );
    } else {
      if (role === "dormant") {
        ...
        if (typeof revisitTrigger !== "string" || revisitTrigger.trim().length === 0) {
          diagnostics.add(
            ...
            `${pPtr}/revisitTrigger`,
            `dormant platform "${platformId}" requires a non-empty revisitTrigger`,
          );
        }
      }
      ...
      if (role === "not-targeted") {
        ...
        if (typeof rationale !== "string" || rationale.trim().length === 0) {
          diagnostics.add(
            "E_NOT_TARGETED_RATIONALE",
            `${pPtr}/rationale`,
            `not-targeted platform "${platformId}" requires rationale`,
          );
        }
      }
    }
    ...
    if (!Array.isArray(suites) || !suites.every((s) => typeof s === "string")) {
      diagnostics.add(
        "E_TYPE",
        `${pPtr}/acceptanceSuites`,
        "acceptanceSuites must be an array of strings",
      );
    }
      ...
      if (guideVal !== undefined) {
        if (typeof guideVal !== "string" || guideVal.trim().length === 0) {
          ...
          diagnostics.add(
            "E_PROVENANCE_INCOMPATIBLE",
            `${pPtr}/${guideKey}`,
            `platform ${platformId} ${guideKey} cannot use mutable branch reference: "${guideVal}"`,
          );
        }
      }
    }
  ...
  return finishResult<ProductOverlay>(value, diagnostics);
}

export function validateTechnologyRegistryOverlay(
  value: unknown,
  pointer = "",
): ValidationResult<TechnologyRegistryOverlay> {
  const diagnostics = new Diagnostics();
  if (
    !diagnostics.object(
      value,
      pointer,
      ["schemaVersion", "contractId", "provenance", "technologies"],
      ["schemaVersion", "contractId", "provenance", "technologies"],
    )
  ) {
    return finishResult(undefined, diagnostics);
  ...
  if (
    ...
    diagnostics.add(
      "E_CONST",
      `${pointer}/schemaVersion`,
      'schemaVersion must equal "technology-registry-overlay/v1" or "1.0.0"',
    );
  }
  ...
  for (const [index, entry] of techsVal.entries()) {
    ...
    if (
      !diagnostics.object(
        entry,
        ePtr,
        ["logicalId", "lifecycle", "priority", "rationale", "package", "selectedVersion", "evaluation"],
        ["logicalId", "lifecycle", "priority", "rationale"],
      )
    ) {
      continue;
    }
    ...
    if (
      ...
      !REGISTRY_LIFECYCLE_VALUES.has(lifecycle as RegistryLifecycle)
    ) {
      diagnostics.add(
        "E_REGISTRY_LIFECYCLE",
        `${ePtr}/lifecycle`,
        `unknown or malformed technology lifecycle value: "${String(lifecycle)}"`,
      );
    }
    ...
    if (evaluation !== undefined) {
      if (
        diagnostics.object(
          evaluation,
          `${ePtr}/evaluation`,
          ["owner", "reviewTrigger", "targetEvidence", "exitCriteria"],
          ["owner", "reviewTrigger", "targetEvidence", "exitCriteria"],
        )
      ) {
  ...
  return finishResult<TechnologyRegistryOverlay>(value, diagnostics);
}

export function validateComponentRegistryOverlay(
  value: unknown,
  pointer = "",
): ValidationResult<ComponentRegistryOverlay> {
  const diagnostics = new Diagnostics();
  if (
    !diagnostics.object(
      value,
      pointer,
      ["schemaVersion", "contractId", "provenance", "components"],
      ["schemaVersion", "contractId", "provenance", "components"],
    )
  ) {
    return finishResult(undefined, diagnostics);
  ...
  if (
    ...
    diagnostics.add(
      "E_CONST",
      `${pointer}/schemaVersion`,
      'schemaVersion must equal "component-registry-overlay/v1" or "1.0.0"',
    );
  }
  ...
  for (const [index, entry] of compsVal.entries()) {
    ...
    if (
      !diagnostics.object(
        entry,
        ePtr,
        ["logicalId", "lifecycle", "priority", "rationale", "package", "selectedVersion"],
        ["logicalId", "lifecycle", "priority", "rationale"],
      )
    ) {
      continue;
    ...
    if (
      ...
      diagnostics.add(
        "E_REGISTRY_LIFECYCLE",
        `${ePtr}/lifecycle`,
        `unknown or malformed component lifecycle value: "${String(lifecycle)}"`,
      );
    }
  ...
  return finishResult<ComponentRegistryOverlay>(value, diagnostics);
}

// Minimal, safe, deterministic YAML parser for overlay documents.
export function parseOverlayYaml(yamlText: string): unknown {
  ...
  function parseBlock(currentIndent: number): unknown {
    ...
    if (firstLine.text.startsWith("- ") || firstLine.text === "-") {
      ...
      while (index < lines.length && lines[index]!.indent === currentIndent) {
        ...
        if (!itemLine.text.startsWith("- ") && itemLine.text !== "-") break;
...
export function formatOverlayYaml(data: unknown, indentLevel = 0): string {
...
export function validateOverlayPayloadEntries(
