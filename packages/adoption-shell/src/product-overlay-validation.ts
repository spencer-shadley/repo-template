...
function checkProvenanceBlock(
  ...
  let found = false;
  for (const field of ["templateRelease", "templateDigest", "templateCommit"] as const) {
...
function checkPlatformEntry(
  ...
  if (typeof role !== "string" || !isPlatformRole(role)) {
    ...
    diagnostics.add(
      "E_INVALID_ROLE",
      `${pointer}/role`,
      `invalid platform role: ${roleStr}; must be one of: ${PRODUCT_PLATFORM_ROLES.join(", ")}`,
    );
    return;
  }
  if (role === "dormant") {
    ...
    if (typeof trigger !== "string" || trigger.trim() === "") {
      diagnostics.add(
        "E_MISSING_DORMANT_REVISIT_TRIGGER",
        `${pointer}/revisitTrigger`,
        "dormant platform requires a non-empty revisitTrigger",
      );
    }
  }
  if (role === "not-targeted") {
    ...
    if (typeof rationale !== "string" || rationale.trim() === "") {
      diagnostics.add(
        "E_MISSING_NOT_TARGETED_RATIONALE",
        `${pointer}/rationale`,
        "not-targeted platform requires a non-empty rationale",
      );
    }
  }
...
export function validateProductOverlay(
  ...
  if (obj === undefined) {
    return finish<ProductOverlay>(undefined, diagnostics);
  }
  ...
  if (!hasEvidence) {
    diagnostics.add(
      "E_REQUIRED_PROVENANCE",
      "",
      "product overlay requires immutable release provenance (version, commit, or digest)",
    );
  }
  ...
  return finish<ProductOverlay>(
    diagnostics.rows.length === 0 ? buildProductOverlay(obj) : undefined,
    diagnostics,
  );
}
...
function checkRegistryEntry(
  ...
  if (typeof state !== "string" || !isRegistryLifecycleState(state)) {
    ...
    diagnostics.add(
      "E_INVALID_LIFECYCLE_STATE",
      `${pointer}/state`,
      `invalid lifecycle state: ${stateStr}; must be one of: ${REGISTRY_LIFECYCLE_STATES.join(", ")}`,
    );
  }
}
...
function checkRegistryOverlayEnvelope(
  ...
  if (kind !== expectedKind && kind !== `${expectedKind}-overlay`) {
    diagnostics.add(
      "E_INVALID_KIND",
      "/registryKind",
      `registryKind must be '${expectedKind}' or '${expectedKind}-overlay'`,
    );
  }
...
export function validateTechnologyRegistryOverlay(
  ...
  const obj = parseOverlayInput(value, diagnostics);
  if (obj === undefined) {
    return finish<TechnologyRegistryOverlay>(undefined, diagnostics);
  }
  ...
  return finish<TechnologyRegistryOverlay>(
    diagnostics.rows.length === 0 ? built : undefined,
    diagnostics,
  );
}
...
export function validateComponentRegistryOverlay(
  ...
  const obj = parseOverlayInput(value, diagnostics);
  if (obj === undefined) {
    return finish<ComponentRegistryOverlay>(undefined, diagnostics);
  }
  ...
  return finish<ComponentRegistryOverlay>(
    diagnostics.rows.length === 0 ? built : undefined,
    diagnostics,
  );
}
