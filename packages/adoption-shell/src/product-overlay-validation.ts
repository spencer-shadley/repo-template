...
function checkPlatformEntry(name: string, def: unknown, diagnostics: Diagnostics): void {
  ...
  if (def["acceptanceSuites"] !== undefined) {
    if (!Array.isArray(def["acceptanceSuites"])) {
      diagnostics.add("E_TYPE", `${pointer}/acceptanceSuites`, "acceptanceSuites must be an array");
    } else {
      for (const [idx, suite] of def["acceptanceSuites"].entries()) {
        if (typeof suite !== "string") {
          diagnostics.add("E_TYPE", `${pointer}/acceptanceSuites/${idx}`, "acceptanceSuite item must be a string");
        }
      }
    }
  }
...
function buildPlatforms(rawPlatforms: unknown): Record<string, ProductPlatformDefinition> {
  ...
  for (const [name, rawDef] of Object.entries(rawPlatforms)) {
    if (isRecord(rawDef) && typeof rawDef["role"] === "string" && isPlatformRole(rawDef["role"])) {
      ...
      if (Array.isArray(rawDef["acceptanceSuites"]) && rawDef["acceptanceSuites"].every((s) => typeof s === "string")) {
...
function checkRegistryEntry(entriesKey: string, id: string, def: unknown, diagnostics: Diagnostics): void {
  ...
  if (def["contexts"] !== undefined) {
    if (!Array.isArray(def["contexts"])) {
      diagnostics.add("E_TYPE", `${pointer}/contexts`, "contexts must be an array");
    } else {
      for (const [idx, ctx] of def["contexts"].entries()) {
        if (typeof ctx !== "string") {
          diagnostics.add("E_TYPE", `${pointer}/contexts/${idx}`, "context item must be a string");
        }
      }
    }
  }
...
function validateRegistryOverlay<T extends TechnologyRegistryOverlay | ComponentRegistryOverlay>(
  value: unknown,
  expectedKind: "technology" | "component",
  entriesKey: "technologies" | "components",
): ValidationResult<T> {
  const diagnostics = new Diagnostics();
  const obj = parseOverlayInput(value, diagnostics);
  if (obj === undefined) return finish<T>(undefined, diagnostics);

  checkRegistryOverlayEnvelope(obj, expectedKind, entriesKey, diagnostics);
  ...
  const built = Object.freeze({
    ...
    ...(typeof obj["templateCommit"] === "string" ? { templateCommit: obj["templateCommit"] } : {}),
    ...(isRecord(obj["provenance"]) ? { provenance: buildProvenance(obj["provenance"]) } : {}),
    [entriesKey]: Object.freeze(buildRegistryEntries(obj[entriesKey])),
  }) as unknown as T;

  return finish<T>(diagnostics.rows.length === 0 ? built : undefined, diagnostics);
}

export function validateTechnologyRegistryOverlay(value: unknown): ValidationResult<TechnologyRegistryOverlay> {
  return validateRegistryOverlay<TechnologyRegistryOverlay>(value, "technology", "technologies");
}

export function validateComponentRegistryOverlay(value: unknown): ValidationResult<ComponentRegistryOverlay> {
  return validateRegistryOverlay<ComponentRegistryOverlay>(value, "component", "components");
}
