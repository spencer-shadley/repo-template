...
const TEST_FILE_PATTERN =
  /(?:\.test|\.spec)\.[cm]?[jt]sx?$|(?:^|[/\\])(?:test|tests|__tests__|e2e|fixtures)[/\\]/i;
...
function isMemberExpression(node: unknown): node is { type: "MemberExpression"; property: unknown } {
  return isRecord(node) && node["type"] === "MemberExpression";
}

function isLiteralString(node: unknown): node is { type: "Literal"; value: string } {
  return isRecord(node) && node["type"] === "Literal" && typeof node["value"] === "string";
}

function isTemplateLiteral(
  node: unknown,
): node is { type: "TemplateLiteral"; quasis: readonly { value?: { raw?: string } }[] } {
  return isRecord(node) && node["type"] === "TemplateLiteral" && Array.isArray(node["quasis"]);
}

function isCallExpression(node: unknown): node is { type: "CallExpression"; callee: unknown } {
  return isRecord(node) && node["type"] === "CallExpression";
}

function isObjectExpression(node: unknown): node is { type: "ObjectExpression"; properties: readonly unknown[] } {
  return isRecord(node) && node["type"] === "ObjectExpression" && Array.isArray(node["properties"]);
}

function isProperty(node: unknown): node is { type: "Property"; key: unknown; value: unknown } {
  return isRecord(node) && node["type"] === "Property";
}

function isSpreadElement(node: unknown): node is { type: "SpreadElement"; argument: unknown } {
  return isRecord(node) && node["type"] === "SpreadElement";
}

function isTestFilename(filename: string | undefined): boolean {
  if (!filename || filename === "<input>" || filename === "<text>") return true;
  return TEST_FILE_PATTERN.test(filename.replaceAll("\\", "/"));
}

function hasPreloadDirective(comments: readonly { value: string }[]): boolean {
  return comments.some((comment) => {
    const text = comment.value.trim();
    return (
      /@hermetic(?:-git)?-preload\b/i.test(text) ||
      /\bhermetic:\s*preload\b/i.test(text)
    );
  });
}

function scriptUsesPreload(script: unknown): boolean {
  if (typeof script !== "string") return false;
  return script.includes("--import") && /repo-quality\/(?:hermetic-)?preload/.test(script);
}

function packageJsonHasPreload(pkgPath: string): boolean {
  try {
    const raw: unknown = JSON.parse(fs.readFileSync(pkgPath, "utf8"));
    if (!isRecord(raw) || !isRecord(raw["scripts"])) return false;
...
function hasPreloadInPackageScripts(filename: string | undefined): boolean {
...
function isGitCommand(arg: unknown): boolean {
...
function isHermeticEnvExpression(expr: unknown): boolean {
  if (isCallExpression(expr)) {
    const name = getCalleeName(expr.callee);
    if (name && HERMETIC_HELPER_NAMES.has(name)) return true;
  }
  ...
  if (isObjectExpression(expr)) {
    return expr.properties.some(
      (prop) => isSpreadElement(prop) && isHermeticEnvExpression(prop.argument),
    );
  }
  return false;
}

function propertyIsHermeticEnv(prop: unknown): boolean {
  if (!isProperty(prop)) return false;
  const key = prop.key;
  const keyName = isIdentifier(key) ? key.name : isLiteralString(key) ? key.value : null;
  if (keyName !== "env") return false;
  return isHermeticEnvExpression(prop.value);
}

function hasHermeticEnvOption(args: readonly unknown[]): boolean {
...
export const hermeticGitSpawnRule: Rule.RuleModule = {
  meta: {
    type: "problem",
    docs: {
      description:
        "Flag spawn/execFile/spawnSync of git in test files that neither pass the helper's env nor run under the preload.",
      recommended: true,
    ...
    messages: {
      unscopedGitSpawn:
        "Spawning 'git' via {{callee}} in test file without hermetic environment. Run tests under '@spencer-shadley/repo-quality/preload' or pass 'env: hermeticGitEnv()' to explicit spawns.",
    },
  },
  create(context) {
