...
import { hermeticGitSpawnRule } from "./hermetic-git-spawn-rule.mjs";
export { hermeticGitEnv, sanitizedGitEnv, cleanGitEnvironment, getHermeticTempDir, isHermeticActive, HERMETIC_GIT_ROUTING_VARIABLES, HERMETIC_MARKER_ENV, HERMETIC_MARKER_ENV_ALIAS, } from "./hermetic-preload.mjs";
 ...
 */
export const STACK_WAIVER_PATTERN = /@stack-waiver\s+id=["']?([a-z0-9][a-z0-9-]{2,63})["']?\s+reason=["']([^"'\r\n]{10,240})["']/i;
export const ESLINT_INLINE_CONFIG_PATTERN = /^\s*eslint(?:-(?:disable(?:-(?:next|line))?|enable|env|global))?(?:\s|$)/i;
export const ISSUE_TRACKING_PATTERN = /\bTODO\b/i;
function isRecord(value) {
    return value !== null && typeof value === "object" && !Array.isArray(value);
}
function hasIssueTrackingReference(text) {
    if (!ISSUE_TRACKING_PATTERN.test(text))
        return false;
...
export const preferTypeScriptRule = {
    meta: {
        type: "problem",
        docs: {
            description: "Require authored source files to be written in TypeScript (.ts/.tsx) per AI-First Engineering Stack standard unless explicitly justified with a reason or stack waiver.",
            recommended: true,
...
export const noEslintInlineConfigRule = {
    meta: {
        type: "problem",
        docs: {
            description: "Disallow inline ESLint configuration; use eslint-suppressions.json or @stack-waiver instead.",
            recommended: true,
        },
        schema: [],
        messages: {
            noInlineConfig: "Inline ESLint configuration is forbidden. Use eslint-suppressions.json for grandfathered lint debt or @stack-waiver for an allowed JavaScript boundary.",
        },
    },
    create(context) {
