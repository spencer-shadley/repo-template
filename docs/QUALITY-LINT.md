...
Adopt the preload by adding `--import @spencer-shadley/repo-quality/preload` to the repository's test script in `package.json`:
...
### What the preload guarantees at process start

1. **Removes git routing variables**: deletes `GIT_DIR`, `GIT_WORK_TREE`, `GIT_INDEX_FILE`, `GIT_COMMON_DIR`, `GIT_OBJECT_DIRECTORY`, `GIT_ALTERNATE_OBJECT_DIRECTORIES`, `GIT_NAMESPACE`, `GIT_PREFIX`, and `GIT_GRAFT_FILE`.
2. **Removes git config injection variables**: deletes `GIT_CONFIG_COUNT`, `GIT_CONFIG_PARAMETERS`, and any `GIT_CONFIG_KEY_*` / `GIT_CONFIG_VALUE_*` pairs.
