...
registry, sorted requested bundle references, and the mandatory no-local-issue-template
conformance flag. `materializer-output-manifest` describes only returned in-memory bytes.
...
`.github/ISSUE_TEMPLATE/` overrides fail closed. V2 modes are `100644` and `100755`; text roles
require valid UTF-8 and binary content requires `generic-base-binary`.
