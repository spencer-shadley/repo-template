...
- **Materialize product/registry overlays with immutable bootstrap provenance:** publish runtime-neutral product and registry overlay contracts (`contracts/product-overlay/v1/product-overlay.schema.json`, `technology-registry.overlay.schema.json`, and `component-registry.overlay.schema.json`) and extend `adoption-shell-v2` with `product-overlay.ts` to materialize portable overlay scaffolding (`product-overlay.yaml`, `technology-registry.overlay.yaml`, `component-registry.overlay.yaml`). Enforce closed platform roles (`primary`, `secondary`, `accessory`, `specialized`, `dormant`, `not-targeted`), mandatory non-empty `revisitTrigger` for dormant surfaces, mandatory non-empty `rationale` for not-targeted surfaces, closed technology/component lifecycle states (`preferred-default`, `context-preferred`, `supported-alternative`, `candidate`, `legacy-compatibility`, `discouraged`, `prohibited`), and fail-closed validation rejecting mutable branch references (`master`, `main`, `HEAD`, branch URLs) in favor of immutable release evidence (SemVer, commit SHA, content digest, or release tag URLs). Ensure byte/digest stability on repeated materialization and strict create-only preservation without mutating consumer repositories or runtime Fleet Registry. MINOR. Fixes #336.
