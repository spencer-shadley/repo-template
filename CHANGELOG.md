...
### Added
- **`@spencer-shadley/repo-quality` 1.12.0 default-branch commit guard (repo-template#440, parent FHR#80):** new `./default-branch-guard` export and `default-branch-guard.mjs` launcher. `install` writes a versioned `pre-commit` and `pre-merge-commit` hook that refuses commits on the default branch (from `origin/HEAD`, falling back to `origin/master`/`origin/main`) of a canonical checkout and names the exit: `git worktree add … origin/<default>` or `fleet-cli git session-worktree`. Linked worktrees, other branches and detached HEAD are unaffected. `check` reports each hook as `current`, `outdated`, `missing` or `foreign` so drift is detectable. Install never overwrites a foreign hook. It is a hook only, never an ACL, read-only attribute or OS lock (code#4094). Adoption and enrollment run `install` once (see `docs/QUALITY-LINT.md`). MINOR.

### Changed
- **Seeded `eslint.config.ts` flags `process.env.X` dot access in all TypeScript (repo-template#435, #452):** a `no-restricted-syntax` rule on `**/*.{ts,mts,cts,tsx}` (plus JS under `scripts/` and `tools/`) asks for `process.env["X"]`. Dot access is TS4111 under `noPropertyAccessFromIndexSignature`, so before this it failed only in `pnpm typecheck`, and a land that skipped typecheck left master red (#434). `pnpm lint` now catches it in `packages/**` and `tests/**` too. `tests/env-dot-access-lint.test.ts` resolves the real config per path and pins the allowed forms (`process.env["X"]`, `process.env[name]`, `{ ...process.env }`, `other.env.X`). Consumers re-syncing the seeded config get the rule. MINOR.
...
### Fixed
- **Enroll hermetic-test-preload in repo-quality typecheck and lint:** add `./hermetic-test-preload/*.ts` to `packages/repo-quality/tsconfig.json`, drop the #433 eslint ignore, and fix the TS4111 env accesses and an unused import it hid. An enrollment test pins both. repo-template#431 slice.
...
- **LocalCiContractV3 declares the fleet PR-validation overlay fields:** `validateLocalCiContractV3` (and the published V3 JSON schema) rejected `prMergeProfileId`, `prBroaderFallback`, `prReceiptBinds{Candidate,Base,Integration}`, `prReceipt`, `fullRequiredLegIds` and `simpleDiff` with `E_UNKNOWN_PROPERTY`, so every real fleet `local-ci.json` (code, cli-wrappers, pool-pacing, this repo) failed strict validation. Each field is now optional but strictly typed (merge-profile id format, booleans, closed `prReceipt` that must agree with the flat flags, unique leg-id list, closed `simpleDiff` classes with reserved `full`/`skipped` names and closed focused commands); other unknown properties remain rejected. Consumers vendoring `artifacts/adoption-shell-v2` (e.g. Code) must re-vendor. PATCH.
