...
### Changed
- **Seeded `eslint.config.ts` flags `process.env.X` dot access (repo-template#435):** a `no-restricted-syntax` rule on `**/*.{ts,mts,cts,tsx}` asks for `process.env["X"]`. Dot access is TS4111 under `noPropertyAccessFromIndexSignature`, so before this it failed only in `pnpm typecheck`; a land that skipped typecheck left master red (#434). `pnpm lint` now catches it, and `lint:env-dot-access:selfcheck` (run in `verify`) proves the rule is configured. Consumers re-syncing the seeded config get the rule. MINOR.
