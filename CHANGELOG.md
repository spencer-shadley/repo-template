...
- **Tip-green after RT#422/#345 (RT#411 land-gate obligation):** Ignore `packages/repo-quality/docs-only-gate/**` and `todo-issue-link/**` in eslint until those trees are enrolled in `packages/repo-quality/tsconfig.json` and lint-clean; add `@stack-waiver` on `todo-issue-link.mjs` and drop unused imports (match `docs-only-gate.mjs`); retarget `build-repo-quality-npm`/`verify-repo-quality-npm` identity pin `1.8.0`→`1.9.1` and require the new launcher exports; regenerate `artifacts/adoption-shell-v2/artifact-manifest.json` so `pnpm build:check` matches the declared closure. Restores declared `pnpm lint`/`pnpm verify` green without `--skip-gate`. PATCH. Fixes #411.
