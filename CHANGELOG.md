...
- **Product and registry overlays with immutable provenance:** make `adoption-shell-v2` materialize portable overlay scaffolding (`product-overlay.yaml`, `technology-registry.overlay.yaml`, `component-registry.overlay.yaml`) and schemas under `contracts/overlays/v1/`. Enforce closed platform role vocabulary (`primary`, `secondary`, `accessory`, `specialized`, `dormant`, `not-targeted`), mandatory `revisitTrigger` on dormant surfaces, `rationale` on not-targeted surfaces, and canonical registry lifecycles (`preferred-default`, `context-preferred`, `supported-alternative`, `candidate`, `legacy-compatibility`, `discouraged`, `prohibited`). Reject mutable branch references (`master`, `main`, `HEAD`, `agent-orchestrator/master`) in bootstrap provenance. Add ADR-0011. MINOR. Fixes #336.
